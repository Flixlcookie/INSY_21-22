...
docker run --name mysql3CHIT -v ~/daten3c/data:/var/lib/mysql -v ~/daten3c/conf:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=geheim -d mysql
...
docker exec -it mysql3CHIT mysql -p
